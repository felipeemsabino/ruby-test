...
class RentalPriceCalculator
	...
	attr_accessor	:rentals_modifications
	
	def initialize(json_cars, json_rentals, json_rentals_modifications)
		create_cars_entites(json_cars)
		create_rentals_entites(json_rentals)
		@rentals_modifications = json_rentals_modifications
	end
	...
	def calculate_rental_price
		...
		rental_modifications_counter = 1
		@rentals.each do |rental|
			@cars.each do |car|
				if car.id == rental.car_id #got the rental and respective car
					
					# current rental 
					rental_information = get_rental_price_information(0, rental, car)
					
					# check if exists modification in current rental
					rental_differences = get_rental_differences(rental)
					
					if rental.id == rental_differences.id
						modified_rental_information = get_rental_price_information(rental_modifications_counter, rental_differences, car)
						update_rental_differences(rental_information, modified_rental_information)
						rentals_prices_array.push(rental_information)
						rental_modifications_counter = rental_modifications_counter + 1
					end					
				end
			end
		end
	...
	def get_rental_price_information(rental_modifications_counter, rental, car)
		...
		return RentalPriceInformation.new(rental_modifications_counter, rental.id, total_rental_prince, commissions, options, actions)
	end
	
	# Update the rental with the values calculated using the new user inputs
	def update_rental_differences(rental_information, modified_rental_information) 
		rental_information.actions.each_with_index  do |rental_action, index|
			if rental_action["amount"] > modified_rental_information.actions[index]["amount"] # in debit
				rental_action["amount"] = rental_action["amount"] - modified_rental_information.actions[index]["amount"]
				rental_action["type"] = rental_action["who"] == "driver" ? "credit" : "debit"
			elsif rental_action["amount"] < modified_rental_information.actions[index]["amount"] # in credit
				rental_action["amount"] = modified_rental_information.actions[index]["amount"] - rental_action["amount"]
				rental_action["type"] = rental_action["who"] == "driver" ? "debit" : "credit"
			end
			
			rental_information.id = modified_rental_information.id
		end
	end
	
	# Return a new rental object with the rental information and it updates
	def get_rental_differences(rental)
		rental_params = Hash.new
		@rentals_modifications.each do |rental_modification|
			if rental.id == rental_modification["rental_id"]
				rental_params["id"] = rental.id
				rental_params["car_id"] = rental.car_id
				rental_params["start_date"] = rental_modification["start_date"] != nil ? rental_modification["start_date"] 	: rental.start_date
				rental_params["end_date"]	= rental_modification["end_date"] 	!= nil ? rental_modification["end_date"] 	: rental.end_date
				rental_params["distance"] 	= rental_modification["distance"] 	!= nil ? rental_modification["distance"] 	: rental.distance
				rental_params["deductible_reduction"] = rental.deductible_reduction
				break
			end
		end
		return Rental.new(rental_params)
	end
